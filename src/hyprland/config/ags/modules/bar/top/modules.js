...
function Launcher() {
	return Box({
		...
		child: Button({
			...
			child: Icon({
				icon: `${App.configDir}/assets/images/fau-logo.png`,
				size: 38,
			}),
		}),
	});
...
function ClientTitle() {
	return Label({
		...
		connections: [[Hyprland, label => {
			...
			const replacements = settings.global.classReplacements;
		...
		child: Box({
			children: [
				Revealer({
					...
					child: EventBox ({
						onHover: () => hoverRevealer = true,
						onHoverLost: () => hoverRevealer = true,
						child: Box({
							css: 'min-width: 140px',
							child: ProgressBar({
								inverted: true,
